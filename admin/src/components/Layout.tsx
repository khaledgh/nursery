...
export function Layout() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50/40">
      <aside className="flex w-64 flex-col bg-white border-r border-slate-200">
        <div className="flex items-center gap-3 px-6 py-6 border-b border-slate-100">
          ...
          />
          <div>
            <span className="block text-[15px] font-extrabold leading-tight text-brand-900">{t("app.name")}</span>
            <span className="block text-[10px] font-bold text-brand-600/80 uppercase tracking-widest mt-0.5">Admin Portal</span>
          </div>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-200 ${
                  isActive
                    ? "bg-brand-50 text-brand-800 shadow-sm shadow-brand-600/5"
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          <button
            onClick={signOut}
            className="w-full flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors"
          >
            <LogOut size={18} />
            {t("nav.logout")}
          </button>
